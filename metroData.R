rm(list=ls())
homePath <- "C:/Users/tnarasimhan/Desktop"
setwd(homePath)
...
### Read in CSV files
rides <- read.csv("metroExport.csv",stringsAsFactors=FALSE)
stations <- read.csv("metroStations.csv",stringsAsFactors=FALSE)
...
myUberMerge <- function(x) {
  stationsMerge <- stations[,14:18]
  dfListBalanced[[x]] <- left_join(dfListBalanced[[x]],stationsMerge,by="line")
}

dfListBalanced <- lapply(1:6, myUberMerge)
names(dfListBalanced) <- myList
